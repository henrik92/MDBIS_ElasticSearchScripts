...
def train_model_and_predict():
    #Y = to_categorical(get_labels(IMG_DIRECTORY), num_classes=NUMBER_OF_CLASSES)
    X = get_images(IMG_DIRECTORY)
    ...
    for i in range(0, labels_length):
       # print('Test Picture ' + str(i+1) + ': estimated class: ' + str(test_labels[i]) + ' || predicted class:' + str(predicted_labels[i]))
